...
```mermaid
flowchart TD
    A[食事] --> B[消化・分解<br/>炭水化物/タンパク質/脂質]
    B --> C[血糖値の上昇]
    C --> D[細胞がグルコースを吸収<br/>細胞から水分が排出]
    D --> E[細胞内外の水分バランスが変化]
    E --> F[組織の電気抵抗が変化]
    F --> G[インピーダンスセンサーで検出<br/>高周波・低周波の2信号]
    G --> H[FLOWアルゴリズムで解析<br/>グルコースカーブの推定]
    H --> I[摂取カロリーを逆算]
    I --> J[アプリ「からだメイト」に表示<br/>消費カロリーとの収支を可視化]
```
...
flowchart LR
    ...
    LF[低周波信号] -->|細胞膜を通過しにくい| ECF
    HF[高周波信号] -->|細胞膜を通過できる| ECF
    HF --> CELL
    ECF --> DIFF[両信号の差分]
    CELL --> DIFF
    DIFF --> DIST[細胞内外の<br/>水分分布を推定]
```
...
```mermaid
flowchart TD
    SIG[検出したい信号<br/>食事による細胞内外の水分移動] --> MEAS[手首でのインピーダンス測定]
    E1[気温・体温<br/>末梢部ほど影響大] --> NOISE[ノイズ・変動要因]
    E2[発汗<br/>皮膚表面のイオン濃度変化] --> NOISE
    E3[湿度<br/>皮膚の水和・電極結露] --> NOISE
    E4[装着状態<br/>電極接触抵抗のばらつき] --> NOISE
    NOISE --> MEAS
    MEAS --> RESULT[摂取カロリー推定値<br/>推定値であり誤差を含む]
```
