...
からだメイト Watchの背面（手首に密着する面）には2種類のセンサーがあります。

| センサー | 役割 |
|---|---|
| 光学式心拍センサー | 心拍数などの計測。消費カロリー算出にも寄与 |
| 生体電気インピーダンスセンサー | 摂取カロリーの自動測定・水分バランスのモニタリング |

摂取カロリー測定の主役は後者の**生体電気インピーダンスセンサー**です。体組成計にも使われる技術で、体にわずかな電流を流し、電気抵抗（インピーダンス）の変化を読み取ります[^1]。
...
高周波と低周波の2種類の信号を使うのは、細胞膜の電気的性質を利用するためです。低周波の電流は細胞膜を通過しにくく主に細胞外液を流れ、高周波は細胞内外の両方を流れます。この差分から細胞内外の水分分布を推定できます。
...
- コアアルゴリズムは米HEALBE社の特許技術「FLOWテクノロジー」で、UC Davisらの検証研究では精度約89%と報告されている
